...
class WebRTCTransportClass {
	constructor(webrtcInstance, { informVisitorCall, informAgent }) {
		this.debug = true;
		this.webrtcInstance = webrtcInstance;
        this.informVisitorCall = informVisitorCall;
	...
	startCall(data) {
		...
		this.informVisitorCall(this.webrtcInstance.room, {
	...
	sendCandidate(data) {
		...
		this.informAgent(data.to, WEB_RTC_EVENTS.CANDIDATE, data);
	}
	...
	sendDescription(data) {
		...
		this.informAgent(data.to, WEB_RTC_EVENTS.DESCRIPTION, data);
	}
...
export class WebRTC {
   ...
   */
	...
	constructor(selfId, room, { informVisitorCall, informAgent }) {
		this.config = {
			iceServers: [],
		};
		this.debug = false;
		...
		this.transport = new this.TransportClass(this, { informVisitorCall, informAgent });
	}
   ...
   */
	...
	getPeerConnection(id) {
		...
		peerConnection.addEventListener('addstream', () => {
			this.updateRemoteItems();
			if(window.userStreamData) {
				this.onRemoteCandidate(window.userStreamData);
			}
		});
   ...
   */
	...
	getLocalUserMedia(callRemote, ...args) {
		...
		const onSuccess = (stream) => {
            ...
            document.querySelector('#localLabel').style = "position: absolute; bottom: 0px; padding-top: 5px; width: 100%; color:#D3D3D3; text-align: center; padding-bottom: 5px; font-weight: bold;"
	...
	onRemoteCandidate(data, ...args) {
		...
		document.querySelector('video#remote').srcObject=this.remoteItems[0].url;
        document.querySelector('#remoteLabel').style = "position: absolute; bottom: 0px; padding-top: 5px; width: 100%; color:#D3D3D3; text-align: center; padding-bottom: 5px; font-weight: bold;"
